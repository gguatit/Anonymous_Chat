...
export class UIManager {
    ...
    formatMessageContent(content) {
        ...
        if (codeBlocks.length === 0 && inlineCodes.length === 0 && this.isLikelyCode(content)) {
            return this.renderCodeBlock(content, '');
        }
        ...
        for (let i = 0; i < codeBlocks.length; i++) {
            ...
            formatted = formatted.replace(`\u200B\u200BCODEBLOCK${i}\u200B\u200B`, this.renderCodeBlock(code, lang));
        }
    ...
    renderCodeBlock(code, lang) {
        ...
        const langLabel = detectedLang ? `<span class="code-block-lang">${this.sanitizeInput(detectedLang)}</span>` : '';
        ...
        const codeHtml = `<div class="code-block-wrapper">
            ...
            <pre class="code-block"><code class="hljs${detectedLang ? ` language-${this.sanitizeInput(detectedLang)}` : ''}">${highlightedCode}</code></pre>
        </div>`;
