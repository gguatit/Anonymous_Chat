...
### 2026년 4월 15일 - 메시지 검색, 이미지 붙여넣기, 이미지 갤러리 추가

#### 새로운 기능
- **메시지 검색**: 상단 돋보기 버튼 또는 `Ctrl+F`로 12시간 이내의 모든 메시지를 서버 사이드에서 검색. 내용, 닉네임, 파일명 다중 키워드 AND 검색 지원. 검색 결과 하이라이트 및 클릭 시 해당 메시지로 스크롤 이동
- **�립보드 이미지 붙여넣기**: `Ctrl+V`로 클립보드에 복사한 이미지를 채팅 입력창에 바로 붙여넣기. 파일명 자동 생성 및 업로드 미리보기 처리
- **이미지 갤러리**: 상단 갤러리 버튼으로 채팅에 공유된 모든 이미지를 그리드 썸네일로 한눈에 열람. 썸네일 클릭 시 라이트박스 전체 화면 보기, 방향키 탐색, 새로고침 회전 애니메이션 지원

#### 서버 사이드 변경
- `GET /api/search?q=검색어&limit=N` 엔드포인트 추가 (Durable Object 내 전체 메시지 대상 검색)

### 2026년 3월 27일 - 공지사항 히스토리 페이지 추가

#### 새로운 기능
- **공지사항 히스토리 페이지**: `/announcements.html` 전용 페이지 신설. 과거 공지를 시간순으로 열람 가능
- **공개 API 엔드포인트 추가**: `/api/announcements` — 인증 없이 공지사항 목록 조회 가능
- **헤더 네비게이션 버튼**: 채팅 화면 상단에 "📢 공지" 버튼 추가로 간편 이동 지원

### 2026년 3월 26일 - 임시 닉네임 및 닉네임 잠금 기능 추가

#### 새로운 기능
- **임시 닉네임**: 사용자가 세션 동안 유지되는 1회성 닉네임 설정 가능. 메시지에 "Anonymous" 대신 설정한 닉네임 표시
- **닉네임 잠금(Padlock)**: 닉네임 입력 필드 기본 잠금 상태, 잠금 해제 클릭 시 면책 모달 표시
- **면책 모달**: 사칭·부적절 콘텐츠 경고 및 "다시 보지 않기" 체크박스(localStorage 영구 상태 저장)
...
- 이미지 인라인 미리보기
- 비디오/오디오 스트리밍 재생
- **이미지 갤러리** (그리드 썸네일 + 라이트박스 전체 화면 보기, 방향키 탐색)
...
| `/api/search` | GET | 메시지 검색 (q, limit 파라미터) |
| `/` | GET | 정적 파일 (HTML) |
...
- **이미지 갤러리**: 채팅 내 이미지 그리드 썸네일 + 라이트박스 전체 화면 보기
  - 방향키 탐색, 새로고침 회전 애니메이션
  - 메시지 내 이미지 클릭 시 갤러리 열기
