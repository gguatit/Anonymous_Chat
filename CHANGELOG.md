...
### 🚀 신규 기능
- **AI 요약 모드**: 4가지 분석 모드 추가 — `/summary`(종합), `/topic`(주제), `/mood`(분위기), `/conflict`(논쟁)
  - 각 모드별 전용 시스템 프롬프트 (9~12개 절대 규칙)
  ...
  - 요약 결과 모든 사용자에게 공유 (indigo 카드)

### 🔧 개선
- AI 레이트 리밋 30초 → 15초로 완화
- AI 시스템 프롬프트 강화: 9개 절대 규칙 (대화 참여 금지, 환각 방지, 개인정보 차단)
- AI 피드백 루프 방지: `/messages/recent`에서 AI 요약 메시지 제외
- temperature 0.7 → 0.4 (응답 일관성 향상)
- `/` 명령어 팝업: 4개 명령어 키보드/클릭 선택 지원

### 🐛 버그 수정
- `_renderSingleMessage`에서 `type:'summary'` 히스토리 배치 렌더링 지원
- AI 요약 중복 출력 방지 (messageId 기반 dedup)
- AI가 채팅에 답장하는 문제 수정 (프롬프트 규칙 #0 추가)
- 레이트 리밋 태그 격리: `checkRateLimit`에 `tag` 파라미터 추가로 엔드포인트 간 독립적 제한

### 📝 문서
- `README.md`: AI 요약 4가지 모드 설명 업데이트
- `FEATURE_IDEAS.md`: #31 AI 대화 요약 모드별 분석 반영
- `help.html`: AI 요약 섹션에 4개 모드 설명 추가
